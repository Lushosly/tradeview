...
comp_ticker = st.sidebar.text_input("Compare Against", value="").upper() # Default empty

st.sidebar.header("⚙️ Timeframe")
# UX IMPROVEMENT: Standard Financial Presets instead of raw numbers
timeframe_options = ["1M", "3M", "6M", "YTD", "1Y", "3Y", "5Y", "Max"]
selected_tf = st.sidebar.selectbox("Range", timeframe_options, index=4) # Default 1Y
...
if df1 is not None and not df1.empty:
    try:
        ...
        # Annualized Volatility logic (handle short timeframes safely)
        ...
        # Safe access to RSI (needs 14 days of data)
        rsi_val = float(df1['RSI'].iloc[-1]) if len(df1) > 14 else 0.0
        ...
        # TAB 3: COMPARISON (Relative Performance)
        with tab3:
            if df2 is not None and not df2.empty:
                ...
                comp_fig.update_layout(title=f"Cumulative Return (%) - {selected_tf}", yaxis_title="Return (%)", template="plotly_dark", height=500, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
                st.plotly_chart(comp_fig, use_container_width=True)
                
                common = df1.index.intersection(df2.index)
                if len(common) > 0:
                    corr = df1['Close'][common].corr(df2['Close'][common])
                    st.info(f"ℹ️ Correlation Coefficient: **{corr:.2f}**")
            else:
                if comp_ticker:
                    st.warning(f"Could not load data for {comp_ticker}. Please check the symbol.")
                else:
                    st.info("Enter a comparison ticker in the sidebar to compare performance.")
...
<div class="disclaimer">
    ...
    The forecasts and indicators are generated by mathematical models (Linear Regression, Moving Averages) and do not constitute financial advice. 
